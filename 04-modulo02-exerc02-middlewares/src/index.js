...
function checksTodoExists(request, response, next) {
  ...
  if (!validate(id)) {
    return response.status(400).json({error: 'the id is not uuid'})
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!validate(id)) {
    return response.status(400).json({ error: "Id format is not a v4 uuid"})
  }
  ...
  if (indexTodo === -1) {
    return response.status(404).json({ error: "Id does not belong to the specified user"})
  }
  request.user = userFound;
  request.todo = userFound.todos[indexTodo];
